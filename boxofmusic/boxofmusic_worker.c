...
#define TAG "boxofMusicWorker"

#define BOXOFMUSIC_FILETYPE "Flipper Music Format"
#define BOXOFMUSIC_VERSION 0
...
static int32_t boxofmusic_worker_thread_callback(void* context) {
    furi_assert(context);
    boxofMusicWorker* instance = context;
...
boxofMusicWorker* boxofmusic_worker_alloc() {
    boxofMusicWorker* instance = malloc(sizeof(boxofMusicWorker));

    NoteBlockArray_init(instance->notes);

    instance->thread = furi_thread_alloc_ex(
        "boxofMusicWorker", 1024, boxofmusic_worker_thread_callback, instance);
...
static bool boxofmusic_worker_add_note(
...
static bool boxofmusic_worker_parse_notes(boxofMusicWorker* instance, const char* string) {
    ...
    while(*cursor != '\0') {
        if(!is_space(*cursor)) {
            ...
            if(boxofmusic_worker_add_note(instance, semitone, duration, dots)) {
                FURI_LOG_D(
                    TAG,
...
bool boxofmusic_worker_load(boxofMusicWorker* instance, const char* file_path) {
    ...
    if(strcasestr(file_path, ".fmf")) {
        ret = boxofmusic_worker_load_fmf_from_file(instance, file_path);
    } else {
        ret = boxofmusic_worker_load_rtttl_from_file(instance, file_path);
    }
    return ret;
}
...
bool boxofmusic_worker_load_fmf_from_file(boxofMusicWorker* instance, const char* file_path) {
    ...
    do {
        ...
        if(furi_string_cmp_str(temp_str, BOXOFMUSIC_FILETYPE) ||
           (version != BOXOFMUSIC_VERSION)) {
        ...
        if(!boxofmusic_worker_parse_notes(instance, furi_string_get_cstr(temp_str))) {
            break;
        }
...
bool boxofmusic_worker_load_rtttl_from_file(boxofMusicWorker* instance, const char* file_path) {
    ...
    do {
        ...
        if(!boxofmusic_worker_load_rtttl_from_string(instance, furi_string_get_cstr(content))) {
...
bool boxofmusic_worker_load_rtttl_from_string(boxofMusicWorker* instance, const char* string) {
    ...
    cursor++;
    if(!boxofmusic_worker_parse_notes(instance, cursor)) {
        return false;
    }
